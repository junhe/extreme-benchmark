...
{
    if ( argc != 6 ) {
        printf("Usage: %s mem_len start_addr op_size stride upsize\n", argv[0]);
    ...
    long mem_len, op_size, start_addr, stride, n_loop, upsize;
    long bytes;
    ...
    for ( padding = start_addr; padding < padding_end; padding+=op_size ) {
        for ( i = 0; i < n_loop && bytes < upsize; ++i, bytes += op_size ) {
